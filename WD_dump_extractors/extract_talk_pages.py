...
def file_extractor(file_name):
    ...
    with bz2.BZ2File(file_name, 'rb') as inputfile:
        ...
        for line in inputfile:
            ...
            if '</revision>' in line:
                ...
                if counter >= 1000:
                    ...
                    try:
                        cur = conn.cursor()
                        cur.executemany("""INSERT INTO revision_pages (comment_rev, item_id, parent_id, rev_id, time_stamp, user_name) VALUES (%(comment)s, %(pageTitle)s, %(parent_id)s, %(rev_id)s, %(time_stamp)s, %(user_name)s);""",revision_processed)
                        conn.commit()
                        # print 'imported'
                    except :
                        conn.rollback()
        ...
        try:
            ...
            cur.executemany(
                """INSERT INTO revision_history (comment_rev, item_id, parent_id, rev_id, time_stamp, user_name) VALUES (%(comment)s, %(pageTitle)s, %(parent_id)s, %(rev_id)s, %(time_stamp)s, %(user_name)s);""", revision_processed)
            conn.commit()
            # print 'imported'
        except:
            conn.rollback()
